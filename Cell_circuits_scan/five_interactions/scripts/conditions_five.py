"""
Stability checks used in the five‑interaction circuit screen.

Given a `Combo_CC` model (`cm`) and a fixed point `fp`, this module:
- computes the Jacobian and performs linear stability analysis of the
  origin (or a general fixed point),
- when linearization is inconclusive (zero real part eigenvalues),
  applies a center‑manifold expansion up to a specified order to decide
  stability,
- optionally checks the sign of the flow in a small neighbourhood of
  the origin across a range of parameter values.

These routines are called when filtering five‑interaction circuits to
ensure that the “OFF” state at (X, Y) = (0, 0) is stable and that no
other stable fixed points persist for all parameter values, matching
the stability criteria described in the Methods.
"""
